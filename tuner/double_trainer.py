...
def test_double(model, testDataloader, scaler_y):
    ...
    data = pd.DataFrame(columns=["knobs_with_info", "True", "Predict"])
    with torch.no_grad():
        for _, batch in enumerate(tqdm(testDataloader, desc="Iteration")):
            ...
            data = data.append(
                {"knobs_with_info": knobs_with_info.numpy()[0], "True": target.numpy()[0][0], "Predict": output.numpy()[0][0]}, ignore_index=True)
    ...
    return test_loss, test_mae, data
    ...
    for i, model in enumerate(model.items()):
        ...
        for epoch in range(int(opt.n_epochs)):
            ...
            train_loss, _ = train_epoch(
                model, trainDataloader, optimizer[model_name])
            logger.info("[Train Epoch {}] train Loss : {}".format(
                epoch+1, train_loss))

            logger.info(
                "====================================Val====================================")
            val_loss, _ = eval_epoch(model, valDataloader)
            logger.info("[Eval Epoch {}] val Loss : {}".format(
                epoch+1, val_loss))

            logger.info(
                "====================================Test====================================")
            test_loss, test_mae, test_data = test(
                model, testDataloader, scaler_y)

            logger.info("[Epoch {}] Test_Loss: {}, Test_MAE : {}".format(
                epoch+1, test_loss, test_mae))

            if test_loss < best_loss:
                # double needs save naming
                torch.save(model.state_dict(), os.path.join(
                    model_save_path, "{}_{}.pt".format(model_name, str(epoch+1))))
                best_loss = test_loss
                best_mae = test_mae
                best_epoch = (epoch + 1)
                patience = 0
            if patience == 5:
                test_data.to_csv(f"./predict_csv/{opt.target}_{opt.persistence}_{model_name}.csv")
                break
